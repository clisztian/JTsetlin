 ...
 *
 */
public class AutomataAtom {

	
	final int INT_SIZE = 32;
	
	private final ConvolutionEncoder encoder;
	private final int number_of_patches;
	private final int nClauses;
	private final int la_chunks;
	private final int state_bits;
	...
	/**
	 * For type-iii feedback
	 */
	private int[][][] ta_state_ind; //[CLAUSES][LA_CHUNKS][STATE_BITS];
	private int[][] clause_and_target;
	private float d; //learning rate for ind
	private int state_bits_ind;
	 ...
	 * @param boost
	 */
	public AutomataAtom(ConvolutionEncoder encoder, int threshold, int nClauses, float max_specificity, boolean boost, float clause_drop_p) {
	...
	public AutomataAtom initialize(int[][][] _ta_state) {
	...
	public AutomataAtom initializeInd(int[][][] _ta_state_ind, float d, int _state_bits) {

		state_bits_ind = _state_bits;
		ta_state_ind = _ta_state_ind;
		clause_and_target = new int[clause_chunks][la_chunks];
		this.d = d;
 ...
 // Increment the states of each of those 32 Tsetlin Automata flagged in the active bit vector.
	...
	private void inc_ind(int clause, int chunk, int active) {

		int carry, carry_next;
		carry = active;
		for (int b = 0; b < state_bits_ind; ++b) {
			if (carry == 0)
				break;

			carry_next = ta_state_ind[clause][chunk][b] & carry; // Sets carry bits (overflow) passing on to next bit
			ta_state_ind[clause][chunk][b] = ta_state_ind[clause][chunk][b] ^ carry; // Performs increments with XOR
			carry = carry_next;
		}

		if (carry > 0) {
			for (int b = 0; b < state_bits_ind; ++b) {
				ta_state_ind[clause][chunk][b] |= carry;
			}
		}
	}
	...
	private void dec_ind(int clause, int chunk, int active) {
	...
	public int sum_up_class_votes_regression() {
		...
		ArrayList<Integer> regression_clauses = new ArrayList<Integer>();
		for (int j = 0; j < nClauses; j++) {
    ...
    public float getSumOfWeights() {
		...
		for (int i = 0; i < nClauses; i++) {
			...
			int clause_chunk = i/ 32;
			int clause_chunk_pos = i % 32;
